...
private let log = Logger(subsystem: "io.nyuu.RevoHostApp.Revo", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<RevoMainView>?
	...
	*/
    ...
    public override func viewDidLoad() {
        ...
        view.layer?.rasterizationScale = view.window?.backingScaleFactor ?? 2.0
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try RevoAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? RevoAudioUnit else {
				log.error("Unable to create RevoAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(RevoParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = RevoMainView(parameterTree: observableParameterTree)
